...
After building OLS and RF factor models to forecast one-month-ahead sector returns, \citeA{simonian_2019} uses Association Rule Learning (ARL) to transform those forecasts, along with volatility indicators, into a transparent trading rule. This paper also examines RuleFit, which also generates the same "if-then" rules as RF+ARL, but does it all in one step.
