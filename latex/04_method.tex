...
\begin{algorithm}[H]
    ...
    Let $\alpha$ and $\beta$ be non-negative thresholds (e.g., $\alpha = 0$, $\beta = 0.02$).
    Let $VR_t$ denote the volatility ratio at time $t$, defined as:
    $$VR_t = \frac{\text{Volatility}(t-5, t)}{\text{Volatility}(t-21, t)}$$
    where near-term volatility is smaller than long-term volatility when $VR_t < 1$.

    \textbf{Input:} Historical sector returns, RF predictions $\hat{r}_{t+1}$, volatility data
    
    \textbf{Output:} Portfolio weights $w_i$ for each sector $i$

    \Begin{
        \textbf{Phase I: Signal Generation}
        
        \For{each time period $t$}{
            Compute volatility ratio: $VR_t = \frac{\text{Volatility}(t-5, t)}{\text{Volatility}(t-21, t)}$\;
            
            Compute RF prediction $\hat{r}_{t+1}$ for one-month-ahead sector return\;
            
            \textbf{Association Rule Check:}
            
            Compute support: $\text{supp}((VR_t < 1, \hat{r}_{t+1} > \alpha)) = \frac{\#\text{ of months }(VR_t < 1, \hat{r}_{t+1} > \alpha)}{\text{Window Length}}$\;
            
            Compute lift: $\text{lift}((VR_t < 1, \hat{r}_{t+1} > \alpha) \Rightarrow r_{t+1} > \beta) = \frac{\text{supp}((VR_t < 1, \hat{r}_{t+1} > \alpha, r_{t+1} > \beta))}{\text{supp}((VR_t < 1, \hat{r}_{t+1} > \alpha)) \times \text{supp}(r_{t+1} > \beta)}$\;
        }
        ...
        \For{each sector $i$ at end of month $t$}{
            \If{sector's $\text{lift}_{(t+1)} > 0$ \textbf{and} $VR_t < 1$ \textbf{and} $\hat{r}_{t+1} > \alpha$}{
                Set signal $s_i = 1$ (positive signal)\;
            }
            \Else{
                Set signal $s_i = 0$ (no signal)\;
            }
        }
        ...
        \If{$N \neq 0$}{
            \For{each sector $i$}{
                \If{$s_i = 1$}{
                    $w_i = \frac{100\%}{N}$ (equal weight among selected sectors)\;
                }
                \Else{
                    $w_i = 0$\;
                }
        ...
        \Else{
            \For{each sector $i$}{
                $w_i = \frac{100\%}{\#\text{Assets}}$ (equal weight across all sectors)\;
            }
        }
...
%todo: constrained and unconstrained: why?

% \subsection{Hypothesis}

% - OLS base : both c4f and ff5 Features significant in prediciting excess returns (EXPLAIN WHY HERE). However will not be explaining variations in the out of sample model, though high R2 in sample. Would not perform good on sector rotation strategy. Unable to adjust to market conditions (risk,liquidity, sentiment) due to OLs linearity nature (parametric) Suspect that martket excess returns will have the most effect and will always be highly statistically significant. {EXPLAIN WHY YOU SUSPECT HIGH MARKET EXCESS COULD LEAD TO HIGH EXCESS RET FOR LINEAR MODELS}. { ARGUE WHICH c4F FACTORS (['excess_mkt_ret','smb', 'hml','umd']) DO YOU THINK WILL HAVE POSITIVE/NEGATIVE EFFECT ON EXCESS RET, AND TO WHAT EXTENT} { ARGUE WHICH ff5 FACTORS ['excess_mkt_ret','smb', 'hml','rmw','cma'] DO YOU THINK WILL HAVE POSITIVE/NEGATIVE EFFECT ON EXCESS RET, AND TO WHAT EXTENT}

% - OLS enhanced : only c4f Features significant in prediciting excess returns (EXPLAIN WHY HERE). However, due to multicollinearity, some features (escpecially features ...) does not work in an OLS (REPHRASE THIS TO BE MORE CLEAR). Therfore expect this to perform even worse than OLS base, in terms of predictive power and variation explained. Though for attribution, Suspect it would be better than OLS base as some biases are accounted for. Still, still expect excess ret to be the most important n varaible in lienar models. However, {ARGUE WHICH VARIABLE YOU SUSPECT WILL BE REMOVED DUE TO MULTICOLLINEARITY AND WHICH WILL BE INSIGNIFICANT 'turn','turn_sd','mvel1','dolvol','daily_illq','zero_trade_ratio','baspread','enhanced_baker','vix_close','put_call_ratio','news_sent'}
%- RF based models:
