...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[scale=1.0]
        ...
        \node[font=\Large\bfseries] at (6, 10) {Random Search with Time Series CV};
...
A downside of RF (and most other ML models) is their “black-box” nature, capturing complex, non-linear relationships between predictors that resist the intuitive loading interpretation of common financial methods such as OLS or PCA. In traditional asset-pricing frameworks, each factor's loading or exposure is represented by its $\beta$, measuring the elasticity of a security's returns with respect to that factor, while the model's intercept $\alpha$ is the number that indicates the percentage difference between the chosen index and the stock's actual performance. Practitioners routinely estimate betas to gauge exposures to market, size, and value factors, and interpret alphas as indicators of manager skill or persistent mispricing. With RF, Relative Feature Importance (RFI) is often employed to rank predictors by their contribution to out-of-sample predictive performance. RFI weighs all of the factors within the RF according to their predictive robustness. \citeA{simonian_2019} used RFI to construct “pseudo-betas”, or RFI-weighted elasticities analogous to traditional factor loadings. An accepted understanding of "betas" in linear models is the elasticity of one variable to another, therefore a RFI-weighted elasticity, or "psuedo-beta" is constructed as follows:
